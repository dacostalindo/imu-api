...
There are currently two examples: The measurement data example
(example_mti1_i2c_spi_receive_measurement_data) and the protocol explorer
(example_mti1_i2c_spi_mtssp_protocol_explorer).
...
Plug the dev board onto the arduino connector of the NUCLEO board. Connect the
NUCLEO board with a USB cable to the PC. Observe that (in Windows) a virtual COM
port and a virtual drive are created for the NUCLEO board. Connect a terminal
emulator to the virtual COM port using a baud rate of 921600, 8 data bits, no
parity bit and 1 stop bit. Please consult the development kit manual [1] for
more information regarding the hardware setup.
